...
const generateReportHtml = (transactions: any[], summary: any, insight: any, generatedDate: string) => {
  ...
  const aiSection = insight && insight.insight 
    ? '<section class="ai-section"><div class="ai-decor"></div><div class="ai-header"><div class="ai-icon"><span class="material-symbols-outlined" style="font-size:24px;color:#9a4021;">psychology</span></div><div class="ai-title">Analisis AI</div></div><div class="ai-content">' + insight.insight + '</div></section>'
    : '';

  const html = '<!DOCTYPE html><html class="light" lang="id">' +
'<head><meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1.0"/><title>Saku POS - Laporan Harian</title>' +
...
'</style></head>' +
'<body><main>' +
'<header><div class="brand"><div class="brand-title">Saku</div><div class="page-title">Laporan Harian</div><div class="meta"><span>' + generatedDate + '</span><span class="dot"></span><span>Generated by Saku POS</span></div></div>' +
'<div class="badge"><span class="material-symbols-outlined badge-icon">auto_awesome</span><span class="badge-text">Daily Report</span></div></header>' +
'<section><div class="section-title">Ringkasan</div><div class="grid">' +
'<div class="card"><div class="card-label">Total Omzet</div><div class="card-value primary">' + formatCurrency(summary.total_omzet || 0) + '</div></div>' +
'<div class="card"><div class="card-label">Transaksi</div><div class="card-value">' + (summary.total_transaksi || 0) + '</div></div>' +
'<div class="card"><div class="card-label">Pending</div><div class="card-value">' + (summary.pending_count || 0) + '</div></div></div></section>' +
'<section><div class="section-title">Transaksi Terbaru</div><table class="transaction-table"><tr><th>Waktu</th><th>Item</th><th style="text-align:right">Total</th></tr>' + rowsHtml + '</table></section>' +
aiSection +
'<footer><span>Saku POS Document</span><span>Page 1 of 1</span></footer></main></body></html>';
